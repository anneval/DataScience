...
babynames %>% filter(0.05>=prop, prop<0.80)  %>% 
  head()

babynames %>% filter(name %in% c("Max","Moritz"))  %>% 
  head()

babynames %>% filter(is.na(name))  %>% 
  head()
...
```{r echo=TRUE}
babynames %>% filter(name == "Max" & sex == "F" ) %>% 
  head()

babynames %>% filter(year == 1990 & n %in% c(5,6)) %>% 
  head()

babynames %>% filter(name=="Max"|name=="Moritz" |name=="Wilhelm") %>% 
  head()
...
Anne %>% ggplot (aes(x=year,y=prop)) + geom_line()
# Enter your solution here
...
### anders!! braucht carrier icht extra
flights2 <- flights %>% 
  left_join(airlines)
...
weighted_mean <- household_clean %>% group_by(cutenure) %>% mutate(pop_weighted_mean = weighted.mean(exp_pc,finlwt21)
)

# finlwt21* as population weight, CU characteristics, income, weights, and summary level expenditures.???
...
```{r echo=TRUE}
weighted_mean %>% ggplot(aes(x=pop_weighted_mean,y=exp_pc,size=cutenure)) + geom_bar()
# Enter your solution here
...
household_clean <- inner_join(household_clean,person_clean, by="newid")

weighted_age <- household_clean %>% group_by(cutenure) %>% mutate(age_weighted_mean = weighted.mean(average_age,finlwt21)
)
...
```{r echo=TRUE}
household_clean %>% ggplot(aes(x=weighted_age,y=, size = cutenure)) + geom_bar()
# Enter your solution here
...
mean_age <- household_clean %>% ceiling(average_age) %>% group_by(cutenure) summarise(average_age = mean(age, na.rm=T)) %>%  mutate(month = paste0("month",month)) %>%  arrange(desc(mean_age)) %>% head(10)
# Enter your solution here
