...
household_clean %>% ggplot(aes(x = average_age, y = Pwccpaha, color = average_age)) + geom_point() + geom_smooth(method=lm)

  geom_bar(mapping = aes(x = average_age, y = Pwccpaha, color = average_age), stat = "summary") + 
  labs(title = "Population weighted per capita consumption per average household age", x = "Average Age", y = "P.c.consumption per average household age") +
  scale_x_discrete(breaks=unique(household_clean$cutenure), labels=addline_format(c("Owned with mortgage","Owned without mortgage","Rented", "Occupied without payment of rent")))  +
  theme_ds
...
```{r}
lm1 <- lm(data=household_clean, Pwccpaha ~ average_age)
summary(lm1)
```
