...
```
A: We can interpret finlwt21 as a population weight based on summary level ependitures and income. 
...
```{r echo=TRUE}
# stat identity summiert falsche Zahlen auf

#ggplot(data = household_clean) +
#  geom_bar(mapping = aes(x = pop_weighted_mean, y = cutenure, fill = cutenure)
...
ggplot(data = household_clean) +
  ...
  coord_flip() +
  theme_classic() + 
  labs(title = "Comparison of the population weighted mean per capita consumption per tenure status", x = "Tenure status", y = "Population weighted mean") + theme(legend.position="top")
...
ggplot(data = household_clean) +
  geom_bar(mapping = aes(x = cutenure, y =age_weighted_mean, fill = cutenure ), stat = "summary") + coord_flip() 
...
household_clean %>% 
  ...
  arrange(household_clean, desc(Pwccpaha)) %>% 
  head(10)

# nicht descending here!!!! ######
...
'ggplot(data = household_clean) +
  geom_bar(mapping = aes(x = average_age, y = Pwccpaha, fill = average_age ), stat = "summary") + coord_flip() 
